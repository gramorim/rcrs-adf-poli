...
public class KMeans extends StaticClustering {
  ...
  public KMeans( AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData ) {
    ...
    this.repeatPreparate = developData
        .getInteger( "poli.module.KMeans.repeatPreparate", 30 );
    ...
    this.assignAgentsFlag = developData
        .getBoolean( "poli.module.KMeans.assignAgentsFlag", true );
  ...
  private void calcStandard( int repeat ) {
    ...
    for ( int i = 0; i < repeat; i++ ) {
      ...
      for ( StandardEntity entity : entityList ) {
        ...
        this.clusterEntitiesList.get( this.centerList.indexOf( tmp ) )
            .add( entity );
      }
      for ( int index = 0; index < this.clusterSize; index++ ) {
        int sumX = 0, sumY = 0;
        for ( StandardEntity entity : this.clusterEntitiesList.get( index ) ) {
          Pair<Integer, Integer> location = this.worldInfo
              .getLocation( entity );
          sumX += location.first();
    ...
    if ( this.assignAgentsFlag ) {
      ...
      List<StandardEntity> ambulanceteamList = new ArrayList<>( this.worldInfo
  ...
  private void calcPathBased( int repeat ) {
    ...
    for ( int i = 0; i < repeat; i++ ) {
      ...
      for ( StandardEntity entity : entityList ) {
        StandardEntity tmp = this.getNearEntity( this.worldInfo,
            this.centerList, entity );
        this.clusterEntitiesList.get( this.centerList.indexOf( tmp ) )
            .add( entity );
      }
